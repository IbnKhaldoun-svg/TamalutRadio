...
- [ ] **Physical UI gate for adjacent placement.** Pending real-device verification that the 16dp adjacent hourglass is clearly readable in both selected and unselected Settings states, does not look clipped/compressed, and does not disturb the four-destination bottom-bar layout. Timer on/off/expiry semantics were already physically validated in the preceding 5/5 gate and remain regression checks.

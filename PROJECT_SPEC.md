...
- [x] Distribution closure is intentionally rebuilt from the exact runtime product commit `1047e60c838099a6a22023b3e24fe2c5476d437d` using the permanent `.github/workflows/publish-debug-release.yml` workflow with explicit `ref=1047e60c838099a6a22023b3e24fe2c5476d437d`, so the public prerelease APK is independent of expired Actions artifact storage. This spec-after commit is documentation-only and does not alter the released runtime bytes.
- [ ] **Physical Stop gate:** PENDING on the published prerelease. Verify mini-player Stop and expanded floating-player Stop for both Radio and local Music, including an active Sleep Timer: audio ends, timer resets Off, media notification and overlay disappear, app is removed from Recents, no ghost reconnect/data activity remains, and launcher relaunch starts a fresh non-resumed session.

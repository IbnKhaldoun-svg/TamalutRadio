...
- [ ] **Home / Recenti / Back-to-home parity.** The floating overlay must become eligible whenever TamalutRadio becomes no longer visible because the user leaves the launcher activity while playback is active, regardless of whether the system transition was initiated by Home, the Recent-apps overview, or Back from the root destination/task. The three navigation paths must share one lifecycle admission path rather than depending exclusively on `Activity.onUserLeaveHint()`.

...
- [x] Add only the official Android Auto media discovery declaration to the phone app: application metadata `com.google.android.gms.car.application` referencing `@xml/automotive_app_desc`, with `<automotiveApp><uses name="media"/></automotiveApp>` in the referenced XML resource.
- [x] Preserve the existing exported Media3 `MediaLibraryService`, legacy `android.media.browse.MediaBrowserService` action, single ExoPlayer/session/service architecture, notification controls, Stop/Exit policy, radio fallback/live reconnect, local Music playback, custom-radio behavior, overlay and Sleep Timer.
- [x] Do not introduce Android for Cars App Library templates, a custom driving UI, a second car/player service, Android Automotive OS packaging, new permissions, or catalog/database changes in this prerequisite.
- [x] Preserve the current minimal browse tree exactly for this baseline certification: `TamalutRadio -> Radio di test -> Radio Azawan / HIT RADIO Maroc / Radio Mars`. Production catalog expansion is not part of this prerequisite and must not be implied by a successful baseline discovery test.
- [x] Automated validation must run `:core:playback:testDebugUnitTest`, `:app:testDebugUnitTest`, and `:app:assembleDebug` on the exact detached product commit, verify the merged APK manifest contains the Android Auto metadata, verify the packaged `automotive_app_desc` declares media support, and verify the persistent debug signer v1 plus APK SHA-256/size before promotion.
- [x] Publish the exact validated runtime as a permanent GitHub debug prerelease before physical testing. Physical baseline gate must verify: TamalutRadio appears in Android Auto's media apps, the `Radio di test` node and its three stations are browsable, selecting a station starts the shared Media3 session, Play/Pause and Previous/Next operate the same queue, metadata stays synchronized with the phone/notification, background/phone interaction does not create double audio, and disconnect/reconnect does not create a second session.
...
Validation record — Android Auto phone-projection discovery prerequisite:
- [x] Spec-before commit: `7885dfcd0ad8a01c407cd395f0554651eab1232d` (`docs: define Android Auto discovery gate`).
- [x] Clean product commit: `362b0a0b86e2fc17229113cf38e0794bd0ea9426` (`feat: declare Android Auto media support`), direct child of the spec-before; exact product diff is only `app/src/main/AndroidManifest.xml` plus `app/src/main/res/xml/automotive_app_desc.xml`.
- [x] Exact detached validation run `33671559012`, job `100386139904`, passed `clean`, `:core:playback:testDebugUnitTest`, `:app:testDebugUnitTest`, and `:app:assembleDebug`: `BUILD SUCCESSFUL in 3m 43s`, 193 actionable tasks (151 executed, 32 from cache, 10 up-to-date).
- [x] Validation verified the merged APK manifest contains `com.google.android.gms.car.application -> @xml/automotive_app_desc`, the APK packages `res/xml/automotive_app_desc.xml`, that descriptor declares `<uses name="media" />`, and the merged service still advertises both Media3 `MediaLibraryService` and legacy `android.media.browse.MediaBrowserService` actions.
- [x] Validation APK: **23,795,589 bytes**; SHA-256 `304c9a096e2d9a4854602f8f4e5c21213b4081f2d7ac828b4936b8aeba6e707a`; persistent debug signer v1 SHA-256 `03225636d52d29f3886592d40747bc85c1c7ad2cafdf622a7d35d409fd928bd6` (`CN=Android Debug, O=TamalutRadio, C=IT`).
- [x] The same successful validation job promoted exactly `362b0a0b86e2fc17229113cf38e0794bd0ea9426` to `main` and removed temporary product/validation branches. Physical Android Auto baseline remains pending until the exact permanent prerelease is exercised on a real Android Auto head unit.

Permanent Release record — Android Auto discovery prerequisite:
- [x] Dispatch helper run `33672250763` requested the permanent publisher with explicit `ref=362b0a0b86e2fc17229113cf38e0794bd0ea9426`; the helper branch removed itself after dispatch.
- [x] Permanent publisher run `33672264843`, job `100388484475`, resolved and detached `HEAD` to exact runtime `362b0a0b86e2fc17229113cf38e0794bd0ea9426` and logged `Building exact commit: 362b0a0b86e2fc17229113cf38e0794bd0ea9426`.
- [x] Publisher `:app:assembleDebug` passed with `BUILD SUCCESSFUL in 3m 18s` (165 actionable tasks: 131 executed, 34 from cache). Persistent debug signer remained `03225636d52d29f3886592d40747bc85c1c7ad2cafdf622a7d35d409fd928bd6` (`CN=Android Debug, O=TamalutRadio, C=IT`).
- [x] GitHub prerelease `debug-20260902-191953-362b0a0` targets exactly `362b0a0b86e2fc17229113cf38e0794bd0ea9426`. Asset `TamalutRadio-debug-362b0a0.apk` is uploaded, **23,795,589 bytes**, digest `sha256:304c9a096e2d9a4854602f8f4e5c21213b4081f2d7ac828b4936b8aeba6e707a`, exactly matching the detached validation APK.
- [x] Release URL: `https://github.com/IbnKhaldoun-svg/TamalutRadio/releases/tag/debug-20260902-191953-362b0a0`. Direct APK: `https://github.com/IbnKhaldoun-svg/TamalutRadio/releases/download/debug-20260902-191953-362b0a0/TamalutRadio-debug-362b0a0.apk`.
- [ ] Physical Android Auto baseline: PENDING on this exact permanent prerelease. This gate certifies discovery and the existing three-station test browse tree only; it does not certify full production Radio/custom/local-Music browsing in Android Auto.


### Backup / restore portable user data — contract

This objective is independent from the still-pending Android Auto physical/DHU gate and may proceed without changing or claiming additional Android Auto compatibility. The goal is a user-controlled, portable backup of TamalutRadio-owned data only; it is not cloud sync and it must not weaken Android storage/security boundaries.

Approved behavior before product code:
- [ ] **Settings-only entry point.** `Impostazioni` adds one compact `Backup e ripristino` section with exactly two explicit actions: `Esporta backup` and `Ripristina backup`. No automatic/background backup, account, cloud service, scheduled worker or silent restore is authorized in v1.
- [ ] **Use the Android Storage Access Framework only.** Export uses the system document creator and import uses the system document picker, so the user chooses the destination/source (including any document provider already available on the device). TamalutRadio must not request broad storage permissions, assume a filesystem path or reintroduce a dedicated Google Drive integration. Suggested export name: `TamalutRadio-backup-YYYYMMDD-HHmmss.json`; MIME type `application/json`.
- [ ] **Versioned, app-identifiable JSON format.** V1 uses a UTF-8 JSON envelope with `format = "tamalutradio-backup"`, integer `formatVersion = 1`, `createdAtEpochMillis`, `customStations`, `favorites` and `portablePreferences`. Future format versions greater than the app supports are rejected before any write; unknown fields inside a supported version may be ignored for forward-compatible additive evolution.
- [ ] **Custom-radio payload.** Export every and only `is_custom=true` station with its existing stable `stationId`, trimmed `name`, exact primary HTTPS stream URL and persisted category. User-defined categories are not stored as a separate list because they remain derived from the categories referenced by custom stations. Built-in station rows and built-in fallback streams are never copied into the backup and can never overwrite the catalog shipped by the restoring app.
- [ ] **Favorites payload.** Export favorites as stable `stationId` plus `favoritedAtEpochMillis`, covering both current built-ins and custom stations. Restore custom stations before favorites. A favorite referencing a current built-in stable ID or a custom station present in the same backup is restored with its saved timestamp. A built-in favorite whose stable ID no longer exists in the restoring catalog is skipped with a visible warning/count rather than creating a fake station. A favorite referring to a missing custom station makes the backup invalid.
- [ ] **Portable preferences payload.** V1 exports/restores `themePreference`, `languageTag`, `overlayEnabled`, `overlayEdge` and `overlayVerticalFraction`. If the backup requests `overlayEnabled=true` but `Settings.canDrawOverlays(...)` is false on the restoring device, keep the restored overlay disabled, preserve the portable edge/position values, and report that the special permission must be granted manually; restore must never open the overlay-permission screen automatically.
- [ ] **Explicit non-portable exclusions.** V1 does **not** export or restore `localFolderUri`, persisted SAF grants, scanned local-music items, `lastPlayed`, the current Media3 queue/session/position, repeat/shuffle runtime state, Sleep Timer state, media-notification state, overlay permission itself, or the mixed `recently_played` history. In particular, a restore must leave the device's currently authorized Music folder/SAF grant untouched rather than clearing or pretending to recreate it.
- [ ] **Plain JSON disclosure.** The v1 backup is intentionally user-readable and not encrypted. The export UI must state concisely that the file may contain the names/categories and stream URLs of user-created radios and should be stored where the user considers appropriate. No app signing material, credentials, tokens, cookies or other TamalutRadio secrets may ever enter the backup.
- [ ] **Bounded preflight before mutation.** Import reads a bounded file (maximum **5 MiB**) and fully validates it in memory before changing Room or DataStore: valid JSON/envelope/version; required fields; unique custom station IDs; unique primary URLs; non-blank names/categories; absolute `https` URL with non-empty host; standard/user-category normalization consistent with the existing custom-radio editor; finite numeric overlay position; and internally consistent favorite references. Malformed, oversized, unsupported-version or conflicting backups fail with a user-visible error and zero intentional data writes.
- [ ] **No network reachability gate during restore.** Restoring a valid backup must not depend on a station being online at that moment. Reuse the existing HTTPS syntax/security invariants, but do not run the add/edit network reachability/HLS probe merely to rehydrate a previously saved custom station. Playback can later exercise the normal stream/fallback behavior.
- [ ] **Deterministic replace semantics after preview.** After successful preflight, show a confirmation summary with the number of custom radios and favorites to be restored and state that **backup-managed data will replace the current backup-managed data**. On confirmation: replace current custom stations/categories and favorites with the backup snapshot; apply the portable preferences snapshot; preserve built-in catalog rows, built-in ordering, local Music folder authorization and active playback state. This is a restore operation, not a merge mode, so repeated restore of the same file is idempotent and cannot create duplicates.
- [ ] **Current-catalog collision protection.** A custom station in the backup must not overwrite a built-in stable ID or duplicate a primary URL already owned by a current built-in. Such a collision is reported during preflight and the restore is rejected before mutation. Built-in catalog count/order therefore remains exactly whatever the restoring app version defines (currently 54) and is never sourced from the backup.
- [ ] **Storage consistency.** All Room replacement work for custom stations and favorites must execute in one explicit Room transaction after preflight. Portable DataStore preferences must be applied in one DataStore edit. Do not claim cross-store ACID atomicity that Room + DataStore cannot provide; unexpected storage/I/O failure must be surfaced as a failure rather than a silent success, and tests must prove validation errors occur before either store is intentionally mutated.
- [ ] **Playback/queue isolation.** Export is read-only. Restore must not stop, restart, seek or rewrite the already-active Media3 session/queue, even if the snapshot removes or changes the currently playing custom station. The existing in-memory queue may finish/continue normally; every future Radio snapshot uses the restored library/categories. No second player, service, session or foreground service is authorized.
- [ ] **Immediate UI projection.** After a successful restore, Radio filters derived from custom categories, custom-station rows, favorites and portable Settings state refresh from their existing repositories/flows without app restart. Deleting the last restored member of a formerly present custom category removes that chip through the existing derived-category behavior.
- [ ] **Required automated coverage.** Add deterministic tests for JSON v1 encode/decode round-trip; export excluding built-ins/non-portable state; custom station/category/favorite round-trip with timestamp preservation; same-file restore idempotence; replace semantics; standard + dynamic category restoration; built-in favorite restoration and unknown-built-in skip reporting; malformed/future/oversized backup rejection; duplicate ID/URL and built-in collision rejection; HTTPS--only validation without network probing; overlay-permission downgrade; local SAF/last-played preservation; Room transaction behavior; DataStore projection; and active-queue/no-second-player regressions. Existing 54-built-in, custom-radio, Music, overlay, Stop and Sleep Timer tests remain green.
- [ ] **Standard validation and distribution closure.** Create one clean product commit directly from this spec-before. Validate the exact detached product in GitHub Actions with `:core:database:testDebugUnitTest`, `:core:data:testDebugUnitTest`, `:core:preferences:testDebugUnitTest`, `:feature:radio:testDebugUnitTest`, `:core:playback:testDebugUnitTest`, `:feature:library:testDebugUnitTest`, `:app:testDebugUnitTest` and `:app:assembleDebug`; verify persistent debug signer v1 plus APK SHA-256/size; promote only the exact validated product to `main`; append docs-only validation/release records; publish the exact runtime through the permanent GitHub debug-prerelease workflow; verify Release target/asset/digest/signer; and remove every temporary helper/product/validation branch.
- [ ] **Physical backup/restore gate.** Before marking this objective complete, exercise the exact permanent prerelease on device: export a snapshot containing at least one custom station in a standard category, one in a user category, built-in + custom favorites and non-default portable preferences; mutate/delete those values; restore the file; verify exact recovery, no duplicates, dynamic-category recovery, local Music folder authorization remains usable, active/background playback remains single-session, and overlay restore respects the device's actual special-permission state. Android Auto certification remains a separate pending gate.

Implementation note for v1: a dedicated serializer/coordinator may be added, but no Room schema migration is required solely for this feature and no new network/cloud dependency is authorized.


### 2026-09-07 — Backup/Restore implementation validated

- Clean product commit: `c062f5890566183a4e5240fca5d502da6908a650` (`feat: add backup restore`), direct child of spec-before `17c216cda0a5130192e2896e25a312b29c22f90d`.
- Backup format is SAF-selected, readable non-encrypted JSON v1 with a 5 MiB import bound. Scope is custom stations/categories, station favorites with timestamps, and portable preferences; local Music SAF authorization, last-played state, playback queue/session and other non-portable state are excluded.
- Import preflight is read/parse/validate only: malformed/future/oversized input, duplicate custom IDs/URLs, built-in ID/URL collisions, invalid categories/preferences, and missing custom-favorite references fail before managed data is replaced. No stream-network probe is performed during restore validation.
- Confirmed restore uses replace/idempotent semantics for backup-managed Room data in one explicit Room transaction; portable DataStore preferences are applied separately without changing the active Media3 playback session. Overlay-enabled restore is downgraded to disabled when the special draw-over permission is unavailable, requiring normal user consent later.
